...
def build():
 ...
 for net,y in [('VDD',770),('VSS',776),('VMID',782)]:d.route(net,'M1',[(2,y),(1770,y)])
 ...
 ports={n:port(d,n,l,x,y) for n,l,x,y in [('a','M1',40,558),('b','M1',40,564),('x','M2',32,392),('cin','M2',769,392),('sum','M1',1520.9,201.3),('cout','M1',1770,64.1),('VDD','M1',1770,770),('VSS','M1',1770,776),('VMID','M1',1770,782)]}
 b=top.dbbox();assert b.left>=0 and b.bottom>=0 and b.right<=1800 and b.top<=1000,b
 d.save('mac',ports,sources,dict(device_counts=dict(PMOS=56,NMOS=56,F_RR=40),embedded_fa_labels_restored=restored,allocation_um=list(ALLOCATION_UM),fits_allocation=True,scope='Core macro without pad frame or interconnect RC extraction'))
