...
def nodesets():
 ...
 lines=['.nodeset v(xdut.p)=5 v(xdut.nmin)=5 v(xdut.x_mul.t2)=5 v(or_out)=-5 v(and_out)=-5',
...
def build():
 ...
 s.label(530,340,'p')
 s.label(400,320,'nmin');s.label(400,360,'or_out')
 ...
 for i,t in enumerate(['B=+5 V: X+A+Cin (ADD/FA); B=-5 V: X-A+Cin (SUB).','X=Cin=0 V: Sum=A*B; Cout=0 (MUL).','Levels -5/0/+5 V encode -1/0/+1. Power: VDD=+5 V, VMID=0 V, VSS=-5 V.','MUL: 14 MOS + 8 RR. FA: 98 MOS + 32 RR. Extra INV: 2 MOS + 2 RR. Total: 114 MOS + 42 RR.','AND=min(A,B)=INV(MUL.t1); OR=max(A,B)=MUL.t3. Total 11 pins including VSS.', 'No internal load capacitors. Add output loading in the TB.']):s.text(40,720+45*i,t)
 s.save('mac')
 ...
 s.text(40,-80,'27 C; +/-5 V and 0 V supplies; 10 fF on all four outputs; 1 ns edges, 200 ns slots.')
 s.comp(str(ROOT/'mac.sym'),400,250,'name=xdut')
 for net,d,dx,dy in pins:s.label(400+dx,250+dy,net)
 s.cap('Csum','sum','10f',740,240);s.cap('Ccout','cout','10f',960,240);s.cap('Cand_out','and_out','10f',740,380);s.cap('Cor_out','or_out','10f',960,380)
 ...
 for j,xval in enumerate((-5,0,5)):
  ...
  for row,state in enumerate(STATES[27*j:27*j+27]):
   ...
   s.text(x,0+32*row,f'{a:+}   {b:+}   {cin:+}    {su:+}    {co:+}    {min(a,b):+}    {max(a,b):+}    {200*k+199}',.22)
 ctrl=start('.options rshunt=1e12\n'+nodesets())+'tran 0.5n 16400n 0 1n\n'
 for k,state in enumerate(seq):
  su,co=oracle(*state)
  for net,target in [('sum',su),('cout',co),('xdut.p',state[1]*state[2]/5),('and_out',min(state[1:3])),('or_out',max(state[1:3]))]:ctrl+=measure(f'tran_{k}_{net.replace(".","_")}',net,200*k+199,target)
