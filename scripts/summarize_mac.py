...
def main():
 ...
 if 'power_join_equivalence' in reports['mac_extracted']:
  lines+=['最終の共有VSS配線40 µm化後にDrawing DRC/LVSと抽出81入力を再実行した。', '匿名ネット1本の名前以外、抽出SPICEは全バイト一致。モデル・参照回路・LVSルールも同一で、全6,480/648遷移の波形を再判定して合格を継承。', '証跡は `reports/mac_improvements/vss_equivalence.json`。配線R/Cは抽出されていないため、拡幅による遅延改善は主張しない。', '']
