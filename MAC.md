...
最終版の抽出回路で、81入力・全6,480遷移・単一入力648遷移を新規実行し、全て合格。全遷移は独立した期待値計算でも波形を再評価した。
包括レビューと5段連結の代表試験は `MAC_FINAL_REVIEW.md` / `reports/final_review/`。5段連結の単体1 µs規定への適用は不可（±0.5 V基準で最大約1.181 µs、代表試験の保持時間は5 µs）。
